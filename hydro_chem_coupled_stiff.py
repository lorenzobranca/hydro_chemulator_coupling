...
net = dde.nn.DeepONetCartesianProd(
    [m, 60, 60, 60],  # Branch network architecture
    [dim_x, 60, 60, 20],  # Trunk network architecture
    activation,
    "Glorot normal",
    num_outputs=3,
...
model_path = 'model_3species_td_multi_stiff/model-10000.ckpt'
...
n_species[:, 0] = 0.25*density  # Initial n1
n_species[:, 1] = 0.25*density  # Initial n2
n_species[:, 2] = 0.5*density  # Initial n3

# ODE system for reference solution
def ode_system(t, n):

    # Robertson-like stiff system with time-dependent coefficients
    # Increased stiffness by introducing large and small reaction rates
    stiff_factor = 1e1  # Controls stiffness

    n1, n2, n3 = n
    conv       = 5e0
    k1 = stiff_factor * (0.04 + 1e7 * np.sin(conv * t))
    k2 = stiff_factor * (3e-1 / (1 + np.exp(-10 * (t - 0.01))))
    k3 = stiff_factor * (1e4 * np.exp(-t))
...
for t in range(nt):
    ...
    for i in range(nx):
        ...
        t_span = [0, dt]
        sol = solve_ivp(ode_system, t_span, initial_conditions, method='BDF')
...
plt.show()
ani.save('hydro_plus_chem_stiff.gif', writer='pillow', fps=5)
